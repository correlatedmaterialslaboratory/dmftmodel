...
class IMP:
	def __init__(self,nom,Nsite,beta,RUUN=True,wbroad=0.03,kbroad=0.15):
		...
		for isite in range(Nsite):
			dir_i = self.dir+str(isite)
			if not os.path.exists(dir_i): os.makedirs(dir_i)
			# creating impurity cix file
			f = open(dir_i+'/'+self.params['cix'][0], 'w')
			print >> f, one_band_cix
			f.close()
		
			root = os.getenv('WIEN_DMFT_ROOT')
			shutil.copy2(root+'/ctqmc', dir_i)
			shutil.copy2(root+'/broad', dir_i)
		
		self.params['nom'][0]=nom
		self.params['beta'][0]=beta
		    
	def Run(self,U,mu_QMC,omega_large,ind_om,isite,Delta,Mstep):
		dir_i = self.dir+str(isite)
		...
		cmd = 'cd '+dir_i+'; '+self.mpi_prefix+' '+self.q_exe+' '+self.PARAMS+' > nohup_imp.out 2>&1 '
		print cmd
		print 'Running ---- ctqmc -----'
		...
		if self.broad:
		    ...
		    cmd = 'cd '+dir_i+'; ./broad -w '+str(self.wbroad)+' -k '+str(self.kbroad)+' '+self.Signame+'w'+' > '+self.Signame
		    print cmd
		...
		for i in range(len(ind_om)):
			Sigs[i,:] = Sig[ind_om[i],:]
		savetxt(dir_i+'/Sig.out',Sigs)
		
		Gf = loadtxt(dir_i+'/Gf.out')
		Gfs = zeros( (len(ind_om), shape(Gf)[1]), dtype=float )
		for i in range(len(ind_om)):
			Gfs[i,:] = Gf[ind_om[i],:]
		savetxt(dir_i+'/Gf.out',Gfs)
		
		savetxt(dir_i+'/Delta.dat',Delta)
		return (transpose(Sigs),transpose(Sig), nf, TrSigmaG)
	
	def copyfiles(self,isite,itt):
		dir_i = self.dir+str(isite)
		shutil.copy2(dir_i+'/'+self.params['Sig'][0], dir_i+'/'+self.params['Sig'][0]+'.'+str(itt))
		shutil.copy2(dir_i+'/Gf.out', dir_i+'/Gf.out.'+str(itt))
		shutil.copy2(dir_i+'/Delta.dat', dir_i+'/Delta.dat.'+str(itt))
		#shutil.copy2(dir_i+'/Gf.out', dir_i+'/nohup_imp.out.'+str(itt))
